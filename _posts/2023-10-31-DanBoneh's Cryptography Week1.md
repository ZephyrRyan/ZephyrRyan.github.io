...
> 1. 根据前面知识点中的发现，我们可以确定大致思路：：随便找任意给定的 m_i，m_j 相与或，如果发现了有意义的英文字母，那么对应位上很可能一个是空格，另一个是英文字母。
> 2. 我们不妨让一个密文与其他 10 条密文异或如果异或出来结果**大部分**都是字母，则说明了这条密文的这个位置对应的明文**极有可能**是个空格。这里的大部分，就是我们代码中空格阈值的由来
> 3. 多次一密，每个明文的对应位置的字节，密钥的字节是相同的，因而只需要选出明文中本是空格的密文，按原来的位置组合，将其与空格异或，就可以得到大致的密钥；  
> 4. 当遍历完十条密文之后，我们就可以得到许多明文是空格的密文的字节，把他们组合起来，与空格异或即可；  
> 5. 有了密钥只需要与密文异或，就得到了明文。  
> 6. 当然，密钥是猜测的，会有瑕疵，因而解出的明文会有少量错误，我们可以人工修正明文，再次与密文异或，便得到了正确的密钥。  
> 7. 由此，所有的密文我们就都可以破解了。
{: .prompt-info }
